...
def createmomana(pagetitle,titlenm, unit, MONTH, ROAM_TYPE, BUSI_TYPE, bv):
    ...
    doddf = pd.read_excel("dod_pptgen.xlsx").sort_values(by='CALLDATE')
    ...
    title = ""
    # valuetuple = (0.135, 0.324, 0.180, 0.135, 0.126, 0.1)
    distprovcdrdf = pd.read_excel("first5_pptgen_prov.xlsx")
...
def GetMonth():
    ...
    def btn_enter_bind(event):
        GetValue()

    root = Tk()

    lab = Label(root, text='Please enter the month')
    ent = Entry(root, bg='white')
    button = Button(root, text='Submit', command=GetValue)
    ...
    root.bind('<Return>', btn_enter_bind)
...
if __name__ == "__main__":
    ...
    # slide1 table 环比表格
    # 获取数据
    df = pd.read_excel("mom_pptgen.xlsx").sort_values(by='MONTH')
    try:
        mom_in_df = df[
    ...
    slide = prs.slides.add_slide(title_only_slide_layout)
    shapes = slide.shapes
    momdf = pd.read_excel("mom_pptgen.xlsx").sort_values(by='MONTH')
    momin = momdf[
    ...
    # 获取值
    carrieropendf = pd.read_excel("carrier_open_4g.xlsx")
